...
- 입고 예정, 검수, 적치 수량을 분리한 이유
- 3PL 환경에서 고객사별 재고 소유권과 로케이션 모델이 중요한 이유
- 출고 지시와 피킹 웨이브를 나눈 이유
- DPS/Print Agent를 중앙 API와 분리한 이유
- 회사 업무 경험을 공개 가능한 Clean-room 프로젝트로 재구성한 방식
