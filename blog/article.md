...
- 입고, 적치, 재고, 출고, 피킹으로 이어지는 WMS 상태 흐름 모델링
- 관리자 웹, API 서버, PDA 앱, DPS/Print Agent를 나눈 멀티 애플리케이션 설계
- 현장 장비와 중앙 시스템의 장애 영향 범위를 분리하는 로컬 에이전트 구조
- 회사 내부 코드와 데이터를 사용하지 않고 실무 도메인을 재구성한 Clean-room 포트폴리오 작성
- 화면, 문서, 실행 단위를 함께 구성해 운영자가 보는 문제와 백엔드 모델을 연결하는 방식
...
이 프로젝트는 물류 도메인을 단순 CRUD로 축소하지 않고, 운영자가 실제로 판단해야 하는 문제를 시스템 구조로 풀어내는 데 초점을 맞췄습니다.

| 운영 문제 | 설계 대응 |
| --- | --- |
| 입고 예정 수량과 실제 검수/적치 수량이 다를 수 있음 | `ReceivingOrderLine`과 `PutawayTask`를 분리해 검수와 적치 상태 추적 |
| 3PL에서는 같은 SKU라도 고객사와 창고에 따라 재고 소유자가 다름 | 고객사, 창고, 로케이션, SKU를 함께 보는 재고 모델 구성 |
| 출고 지시가 여러 채널에서 들어와도 WMS는 재고 할당 기준을 일관되게 가져야 함 | 출고 지시 헤더/라인을 분리하고 할당/피킹 수량을 별도로 관리 |
| 피킹 작업은 운영 화면, PDA, DPS 장비에서 서로 다른 관점으로 보임 | `PickingWave`와 `PickingTask`를 나누고 각 실행 단위가 같은 상태를 바라보게 설계 |
| 현장 장비 장애가 중앙 서비스 전체 장애로 번지면 안 됨 | DPS/Print Agent를 로컬 에이전트로 분리해 장비 연동 경계를 명확히 함 |
...
| 설계 선택 | 이유 |
| --- | --- |
| Admin Web, API, PDA, Agent 분리 | 운영자, 현장 작업자, 장비 연동 프로그램의 배포 위치와 장애 영향 범위가 다르기 때문에 |
| 입고/적치/출고/피킹 상태 분리 | 물류 업무는 한 번의 상태 변경으로 끝나지 않고 단계별 작업 이력이 필요하기 때문에 |
| DPS Protocol Agent 구현 | 실제 장비 없이도 장비 프로토콜, 연결 상태, 완료 이벤트 흐름을 설명하기 위해 |
| Print Agent 분리 | 프린터는 서버 인프라보다 센터 PC와 로컬 네트워크에 가까운 문제이기 때문에 |
| Clean-room 포트폴리오 | 회사 코드와 데이터를 사용하지 않으면서도 실무 도메인 이해를 보여주기 위해 |

이 프로젝트는 “물류 화면을 만들었다”보다 “복잡한 운영 상태와 현장 장비 경계를 어떻게 모델링했는가”를 보여주는 포트폴리오입니다.

## 검증과 게시 구조

이 프로젝트는 물류 상태 흐름이 화면과 API에서 같은 의미로 이어지는지가 중요합니다. 그래서 도메인 기능, 프론트엔드 타입 검증, 포트폴리오 패키징을 나눠 확인합니다.

| 검증 항목 | 확인 내용 |
| --- | --- |
| 도메인 흐름 | 입고, 적치, 재고, 출고 지시, 피킹 웨이브 상태가 단계별로 이어지는지 확인 |
| 화면/계약 | Admin Web과 shared contracts 타입 검증으로 API 응답 구조와 화면 사용 방식을 확인 |
| 포트폴리오 패키지 | 본문, 화면 이미지, manifest가 S3 `portfolio-feed/warehouse-ops-suite/` 구조로 묶이는지 확인 |
...
게시물 업로드는 수동 GitHub Actions로만 실행해, 글과 화면 캡처가 정리된 시점에 공개 feed에 반영하도록 했습니다.
